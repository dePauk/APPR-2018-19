...
ggplot(data=podatki_ekipe_imensko, aes(x=podatki_ekipe_imensko$Uspesnost_redni_del, y=podatki_ekipe_imensko$Playoff_uspesnost)) + geom_jitter(size=(podatki_ekipe_imensko$Stevilo_playoffov)/1, shape=19, color="dodgerblue3") + 
  ggtitle("Korelacija uspešnosti") + xlab("Uspešnost v rednem delu") + ylab("Uspešnost v Play-offih")
```


```{r grafvsdva, echo=FALSE, message=FALSE, fig.align='center', fig.cap='Uspešnost ekip v rednem delu VS uspešnost ekip v Play-Ofssfih'}

#ggplot(data=statistika, aes(x=statistika$Starost, y=statistika$Odigrane_minute)) + geom_smooth(model=lm) + ggtitle("Starost in odigrane #minute") + xlab("Starost") + ylab("Odigrane minute")
```
