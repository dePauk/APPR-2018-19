...
```{r vizualizacija, echo=FALSE, message=FALSE, fig.align='center', fig.cap='Število naslovov igralca tedna po zveznih državah', warning = FALSE}
print(graf_zda)

```

Največ nazivov so pričakovano dobili igralci, ki igrajo za ekipe iz zveznih držav Kalifornija in Texas, saj tam domuje več uspešnih ekip.



```{r vizualizacija, echo=FALSE, message=FALSE, fig.align='center', fig.cap='Uspešnost ekip v rednem delu VS uspešnost ekip v Play-Offih', warning = FALSE}
print(graf_korelacija_uspesn)

```


Opazna je ne preveč močna korelacija med uspešnostjo ekip v rednem delu in v končnici.



```{r vizualizacija, echo=FALSE, message=FALSE, fig.align='center', fig.cap='Število nazivov glede na število sezon v ligi NBA'}
print(graf_sezone)

```

Iz grafa je razvidno, da igralci v povprečju dosegajo vrhunec od 5. do 7. sezone v ligi. Nadpovprečno število nazivov osvajajo od svoje tretje do desete sezone.



```{r vizualizacija, echo=FALSE, message=FALSE, fig.align='center', fig.cap='Višina igralcev glede na pozicijo igranja'}
print (graf_visina_pozicija)

```

Po pričakovanjih se v povprečju višine nagrajenih igralcev povečujejo z bolj napadalno pozicijo igranja.



```{r vizualizacija, echo=FALSE, message=FALSE, fig.align='center', fig.cap='Osvajanje nazivov igralca tedna glede na pozicijo skozi čas'}
print(graf_pozicije_cas)

```

Vidimo, da je trend v zadnjih desetletjih tak, da naslove vedno bolj pogosto osvajajo obrambne pozicije, pomembnost centrov pa je vedno manjša. Tako pravi tudi spodnja napoved za naslednje štiri sezone.
...
```{r vizualizacija, echo=FALSE, message=FALSE, fig.align='center', fig.cap='Primerjava in napoved za dva igralca'}
print (graf_harden_curry2)

```

Iz grafa se vidi, da je Stephen Curry že v zatonu, medtem ko se James Harden še vedno vzpenja po že devetih odigranih sezonah. Za vsakega igralca sta prikazani dve različni grobi napovedi osvojenega števila nazivov igralca tedna za naslednjo sezono.



```{r vizualizacija, echo=FALSE, message=FALSE, fig.align='center', fig.cap='Število zmag vs Število osvojenih nazivov igralca tedna po ekipah'}
print (graf_zmage_nazivi)

```


Vidna je korelacija. V splošnem so igralci ekip, ki so v izbranih sezonah več zmagovale, tudi prejemali več nazivov igralca tedna. Je pa opaznih nekaj izrazitih izjem.
